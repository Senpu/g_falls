...
public class G_falls extends ApplicationAdapter {
	...
	public void render() {
		...
		batch.begin();
		batch.draw(bg_img, 0, 0, Gdx.graphics.getWidth(), Gdx.graphics.getHeight());
		batch.draw(heroImage, rec_hero.x, rec_hero.y);
		for (Rectangle firedrop : firedrops) {
			batch.draw(fireImage, firedrop.x, firedrop.y);
		}
		batch.end();
